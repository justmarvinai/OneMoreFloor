...
**Curses, in the panel body (fifth polish round).** The one control in the game that makes the tower *harder* sits under the preview rather than in the footer: the preview already says what a floor imposes and what it pays, and a curse changes both. Chips are red rather than gold when taken, because this is the one purchase that costs the player something. Below level 100 the whole block is shown with the level that opens it and every chip explains itself on hover (§20.5) — a choice a player cannot see is a choice they cannot climb toward.
...
**Saved gear sets (fifth polish round)** live under the paperdoll that fills them, three cards wide, each with a name field and two explicit buttons. Wear and Save are separate rather than one clever gesture: a click that overwrites a saved set when the player meant to put it on is a loss with no undo. An empty set's Wear is disabled *and says what would fill it*.
...
**Salvage and reforge (fifth polish round).** The gear dialog now answers three questions about a piece you are keeping — level, ascend, reforge, as tabs — and two about one you are not: sell, salvage, side by side in the action row. The salvage button carries the *yield* rather than the word, because salvage is deterministic precisely so it can be compared to the gold beside it before either is pressed. The open tab survives a redraw, since every one of these actions rebuilds the dialog and a tab that snapped back to Upgrade would fight a player doing two reforges in a row.
